...
class OrdersController:
    ...
    async def save(cls, payload: OrderInbound):
        ...
        for item in payload.items or []:
            ...
            if not product_data.data:
                ...
                return CommonMessageOutbound(message=ErrorMessage.INVALID_ID.format("product_id"))
            if product_data.data.get("stock") < item.quantity:
                OrderModel.delete(_id=order.id)
                return CommonMessageOutbound(message=ErrorMessage.STOCKS_NOT_AVAILABLE)
    ...
    async def get(cls, _id: int = None, page: int = 1, size: int = 10):
        ...
        if _id:
            ...
            return CommonMessageOutbound(message=ErrorMessage.FETCH_SUCCESSFULLY, data=data.__dict__)
        else:
            order_response = []
            ...
            return CommonMessageOutbound(message=ErrorMessage.FETCH_SUCCESSFULLY, data=order_response)
